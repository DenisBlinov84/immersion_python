...
def fill(money: int) -> int:
    ...
    __OPERATION_LIST__.append(f'Пополнение наличных на {money_to_fill} у.е.')
...
def get_money(money: int) -> int:
    ...
    __OPERATION_LIST__.append(f'Снятие {money_with_percent} у.е. (с учетом процента от банка) ')
...
def you_wont_be_rich(money: int) -> int:
    ...
    if money > 5_000_000:
        money *= 0.9
        __OPERATION_LIST__.append(f'Снятие процента, так как у вас слишком много денег.')
    return money
...
def check_counter(counter: int, money: int) -> tuple:
    ...
    if counter == 3:
        money *= 1.03
        counter = 0
    __OPERATION_LIST__.append(f'Проверка количества операций. У вас {3 - counter} операций до бонуса.')
